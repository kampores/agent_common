...
class ToolParser:
    ...
    def eval(self, template_str: Optional[str], context_dict: Optional[Dict[str, Any]] = None) -> Optional[str]:
        ...
        def _replace_placeholder(match_obj: re.Match) -> str:
            token_str = match_obj.group(1).strip()
            # 파이프(|) 폴백 지원: {key1|key2|'default'}
            for sub_token in token_str.split("|"):
                sub_clean = sub_token.strip()
                if (sub_clean.startswith("'") and sub_clean.endswith("'")) or (sub_clean.startswith('"') and sub_clean.endswith('"')):
                    return sub_clean[1:-1]

                # 함수 호출 토큰인 경우
                func_m = re.match(r"^([a-zA-Z0-9_]+)\((.*)\)$", sub_clean)
                if func_m:
                    f_name = func_m.group(1)
                    if self.load_tool_function(f_name):
                        try:
                            f_res = self.execute_tool_call(f_name, [], {"ctx": merged_ctx})
                            if f_res is not None and str(f_res) != "":
                                return str(f_res)
                        except Exception:
                            pass

                # 네임스페이스 점 접근 탐색
                val = self._get_ctx_val(merged_ctx, sub_clean)
                if val is not None and str(val) != "":
                    return str(val)

            return ""

        result_str = re.sub(r"\{([^{}]+)\}", _replace_placeholder, tmpl_val_str)
        return result_str
